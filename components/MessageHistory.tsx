    ...
    <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
      <div className="mb-4 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        ...
        <p className="text-sm text-slate-500">
          {orderedMessages.length} saved{" "}
          {orderedMessages.length === 1 ? "entry" : "entries"}
        </p>
      </div>
      ...
      {orderedMessages.length === 0 ? (
        ...
        <div className="relative space-y-4 before:absolute before:bottom-2 before:left-3 before:top-2 before:w-px before:bg-slate-200">
          {orderedMessages.map((message) => (
            <div
              key={message.id}
              className="relative pl-9"
            >
              <div className="absolute left-0 top-1 h-6 w-6 rounded-full border-4 border-white bg-slate-300 shadow-sm" />

              <details className="rounded-xl border border-slate-200 bg-slate-50 p-3 open:bg-white">
                ...
                <div className="mt-3">
                  ...
                  <p className="whitespace-pre-wrap rounded-xl bg-white p-3 text-sm leading-6 text-slate-700 ring-1 ring-slate-200">
                    {message.body || ""}
                  </p>
                </div>
              </details>
